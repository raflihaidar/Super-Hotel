<template>
    <div>
        <NavbarComponnet />
        <section class="my-20 w-[80%] mx-auto">
            <div class="grid grid-cols-1 w-full gap-y-5">
                <div v-for="(item, index) in category" :key="index"
                    ...
                    <span class="flex flex-col gap-y-2 w-[35%]">
                        <p class="text-2xl font-semibold">{{ item.kategori }}</p>
                        <p class="text-xs">{{ item.fasilitas }}</p>
                        <p>{{ item.deskripsi }}</p>
                        <RatingComponent />
                    </span>
                    <span class="flex flex-col items-end gap-y-10 w-[35%]">
                        <span>
                        ...
                        <button type="button" data-modal-target="crud-modal" data-modal-toggle="crud-modal"
                            ...
                            class="text-white items-end bg-green-700 hover:bg-green-800 focus:outline-none focus:ring-4 focus:ring-green-300 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Booking</button>
                    </span>
                </div>
            </div>
        </section>
