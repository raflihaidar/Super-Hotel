    ...
    <div class="w-[80%] mx-auto">
        <form action="" class="grid grid-cols-1 gap-y-3" @submit.prevent="addData" enctype="multipart/form-data">
            ...
            <div class="flex gap-x-4">
                <DropSearchComponent id="divisi" label="divisi" :value="dropDownDivisi">
                    <template #data>
                        <span v-for="(item, index) in divisi" :key="index" @click="addDivisiId(item)">
                ...
                <DropSearchComponent id="shift" label="shift" :value="dropDownShift">
                    <template #data>
                        <span v-for="(item, index) in shift" :key="index" @click="addShiftId(item)">
                            <div class="flex items-center ps-2 rounded hover:bg-gray-100 dark:hover:bg-gray-600">
                                <label for="checkbox-item-11"
                                    class="w-full py-2 ms-2 text-sm font-medium text-gray-900 rounded dark:text-gray-300">
                                    {{ item.nama }}</label>
